...
│       ├── app.js
│       └── styles.css
├── src/                    # 现有原型实现，后续可逐步降级为 legacy/
├── locales/
└── package.json            # 保留为构建辅助入口
```
...
- `/api/status`：返回运行时计划 JSON
- `/api/modules`：返回模块职责 JSON
- `/api/tree`：返回目录树 JSON
- `/`、`/app.js`、`/styles.css`：返回静态 UI
...
pub fn runtime_plan() -> RuntimePlan {
    RuntimePlan {
        memory_target_mb: 10,
        web_boundary: "HTTP/JSON on 127.0.0.1",
    }
}
```
...
async fn main() {
    hyper_serve("127.0.0.1:7788", |req| match req.path() {
        "/api/status" => json(gdp_core::runtime_plan()),
        "/" => html(INDEX_HTML),
        _ => not_found(),
    }).await;
}
...
document.querySelector('#app').textContent = `目标常驻内存 < ${status.memory_target_mb}MB`
```
...
1. **先让 `gdp-core` 独立可编译**
2. 再补 `gdp-cli`，作为最小管理入口
3. 最后补 `gdp-web + rust/ui`，只做只读状态页
4. 等 Rust 路径稳定后，再把现有 Bun/Electrobun 代码逐步降级为兼容层或迁出
...
- Rust workspace 骨架
- `gdp-core` / `gdp-cli` / `gdp-web` 最小实现
- 无框架静态 UI
- Cargo release 优化配置

这意味着后续可以直接沿着 `rust/` 目录继续演进，而不需要重新开新仓库。
