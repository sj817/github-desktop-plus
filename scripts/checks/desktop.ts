...
async function main() {
  ...
  try {
    ...
    devChild = spawnCommand("cargo", ["run", "-p", "gdp", "--", "dev"]);
    ...
    await waitFor(
      async () => {
        if (childExitCode !== null) {
          throw new Error(`gdp dev process exited early with code ${childExitCode}`);
        }
        return wsUrl;
      },
      40000,
    ...
    assert.ok(
      menuState.gdp?.items.some(item => item.id === "gdp.open"),
      "GDP menu should include the settings-dialog action"
    );
