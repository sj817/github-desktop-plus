...
/**
 * Set by scripts/dev.ts to the Vite dev server's address. When present the
...
function setupRendererI18n(
  ...
  const rendererPath = _path.join(dir, 'preload', 'renderer.js')
  if (!_fs.existsSync(rendererPath)) {
    gdpLog(`Renderer bundle not found: ${rendererPath}`, 'error', 'system')
    return []
  }
  ...
  let rendererCode: string
  try {
    rendererCode = _fs.readFileSync(rendererPath, 'utf-8')
  } catch {
    gdpLog(`Cannot read renderer bundle: ${rendererPath}`, 'error', 'system')
    return []
  }

  // The early bundle is also available as an executeJavaScript fallback for
  // Electron builds where session preload registration is unavailable.
  const earlyPath = _path.join(dir, 'preload', 'early.js')
  let earlyCode = ''
  try {
    if (_fs.existsSync(earlyPath)) {
      earlyCode = _fs.readFileSync(earlyPath, 'utf-8')
      gdpLog('Early renderer bundle loaded', 'info', 'system')
    }
  } catch { /* optional */ }
  ...
  const buildEarlyInjectScript = () => earlyCode
  ...
  app.on('browser-window-created', (...args: unknown[]) => {
    ...
    gdpLog('browser-window-created — attaching GDP renderer bundles', 'info', 'system')
...
// ---------------------------------------------------------------------------
// 5c. Early preload — register early.js as a REAL Electron
...
function setupEarlyPreload(
  ...
  const sourcePath = _path.join(dir, 'preload', 'early.js')
...
function main(): void {
  ...
  // 3. Late renderer bundle injection
