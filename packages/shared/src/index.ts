/**
 * The contract between the GDP main process (`packages/hooks/src/ipc.ts`), the dialog
 * shell that lives in GitHub Desktop's renderer (`packages/hooks/src/preload/gdp-dialog`)
 * and the settings UI (`apps/settings-ui`).
 *
...
/**
 * Nested shape — matches the Rust `Config` (crates/gdp-core/src/config.rs) and the
 ...
 * argument tuple and result type. Handlers live in `packages/hooks/src/ipc.ts`.
 */
