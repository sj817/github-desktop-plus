...
/**
 * Port the dev server binds to. The dialog shell reads the same value from
 * GDP_SETTINGS_DEV_URL (set by scripts/dev.mjs), so keep them in sync.
 */
const DEV_PORT = 5273

/** Must match GDP_SETTINGS_UI_GLOBAL in src/shared/gdp-ipc.ts. */
const GLOBAL_NAME = '__GDP_SETTINGS_UI__'
...
export default defineConfig(({ command }) => ({
  ...
  server: {
    ...
    strictPort: true,
    host: '127.0.0.1',
    // The IPC contract lives in ../shared, outside this Vite root.
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        // explicitly.
        footer: `globalThis.${GLOBAL_NAME}=${GLOBAL_NAME};`,
      },
    },
  },
}))
